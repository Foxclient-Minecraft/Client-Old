...
import foxclient.ServidoresDestacados;
...
public class ServerList
{
    ...
    private void loadFeaturedServers() {
		this.addServerData(new ServidoresDestacados("Caraotacraft Network!", "caraotacraft.top"));
		this.addServerData(new ServidoresDestacados("Nigxht Network!", "mc.nigxht.us"));
    }
		
    public int getFeaturedServerCount() {
    	int count = 0;
    	for(ServerData sd: this.servers) {
    		if(sd instanceof ServidoresDestacados) {
    			count++;
    		}
    	}
    	return count;
    ...
    public void saveServerList()
    {
        try
        {
            ...
            {
            	if(!(serverdata instanceof ServidoresDestacados)) {
