...
import foxclient.util.GuiUtil;
import foxclient.util.Render;
...
public class ModKeystrokes extends ModDraggable
{
    ...
    public ModKeystrokes() {
        this.mode = KeystrokesMode.WASD_SPACE_MOUSE;
    ...
    public void render(final ScreenPosition pos) {
        ...
        for (int length = (keys = this.mode.getKeys()).length, i = 0; i < length; ++i) {
            ...
            Render.drawChromaString(key.getName(), pos.getAbsoluteX() + key.getX() + key.getWidth() / 2 - textWidth / 2, pos.getAbsoluteY() + key.getY() + key.getHeight() / 2 - 4, false);
            if (key.cps) {
                ...
                if (key.getName().matches(Key.LMB.getName())) {
                    Render.drawChromaString(" " + this.getCPS() + " CPS", pos.getAbsoluteX() + key.getX() + key.getWidth() / 2 - textWidth / 2, pos.getAbsoluteY() + key.getY() + key.getHeight() / 2 + 4, false);
                }
                if (key.getName().matches(Key.RMB.getName())) {
                    Render.drawChromaString(" " + this.getCPS2() + " CPS", pos.getAbsoluteX() + key.getX() + key.getWidth() / 2 - textWidth / 2, pos.getAbsoluteY() + key.getY() + key.getHeight() / 2 + 4, false);
                }
    ...
    public enum KeystrokesMode
    {
        ...
        WASD_SPACE("WASD_SPACE", 2, new Key[] { Key.W, Key.A, Key.S, Key.D, new Key("\u2015\u2015\u2015\u2015\u2015\u2015", Minecraft.getMinecraft().gameSettings.keyBindJump, 1, 41, 58, 18, false) }), 
        WASD_SPACE_MOUSE("WASD_SPACE_MOUSE", 3, new Key[] { Key.W, Key.A, Key.S, Key.D, Key.LMB, Key.RMB, new Key("\u2015\u2015\u2015\u2015\u2015\u2015", Minecraft.getMinecraft().gameSettings.keyBindJump, 1, 61, 58, 18, false) });
